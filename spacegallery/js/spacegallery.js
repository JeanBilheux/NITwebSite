...
(function($) {
	...
	function nextDescription() {
		...
		if(DescriptionCounter >= NumberSlide) {
			DescriptionCounter = 1;
		} else {
			DescriptionCounter++;
		}
		var description = 'description' + DescriptionCounter;
		$('.description').hide();
		...
		spacegallery : {
			...
			next : function(e) {
				...
				if(EYE.spacegallery.animated === false) {
					...
					//this will display the next description;
					nextDescription();
				}
			...
			init : function(opt) {
				...
				return this.each(function() {
					...
					if($(el).is('.spacegallery')) {
						$('<a href="#tab"></a>').appendTo(this).addClass(opt.loadingClass).bind('click', EYE.spacegallery.next);
